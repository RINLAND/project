        ...
        <div class="container p10 bg-white">
            <div class="board-insert">
                    ...
                    <table>
                        ...
                        <tr>
                            ...
                            <td class="td-40 p-lr3">
                                <c:if test="${freeArticle.division eq 'Y' }">
                        			<span class="notice">공지사항</span>
	                        	</c:if>
	                        	<c:if test="${freeArticle.division eq 'N' }">
	                        		일반
	                        	</c:if>
                            </td>
                        ...
                        <tr>
                            ...
                            <td colspan="3"  class="td-90 p-lr3">
                                <strong>${freeArticle.subject }</strong>
                            </td>
                        ...
                        <tr>
                            ...
                            <td colspan="3"  class="td-90 p-lr3">
                               <strong>${freeArticle.writer }</strong>
                            </td>
                        ...
                        <tr>
                            ...
                            <td colspan="3"  class="td-90 p-lr3 " >
                               <c:if test="${freeArticle.fileOriName eq '' }">
                               	<strong>첨부파일이 없습니다.</strong>
                               </c:if>
                               
                               <c:if test="${freeArticle.fileOriName ne '' }">
                               	<a href="${pageContext.request.contextPath }/freeArticle/grp_article_download?boardCode=${boardCode}&=${view.aid}"><strong>${view.fileOriName }</strong></a>
                               </c:if>
                            </td>
                    ...
                    <div class="btn-grp center m-t10 flex flex-justify">
                        <div class="btn-left">
                        <button type="button" onclick="location.href='${pageContext.request.contextPath }/freeArticle/grp_article_list?boardCode=${boardCode }'" class="btn-red">게시글목록</button>

                        </div>
                        <div class="btn-right">                           
                            <button type="button" class="btn-on"	onClick="location.href='${pageContext.request.contextPath}/freeArticle/grp_article_modify?boardCode=${boardCode}&aid=${freeArticle.aid}'">게시글수정</button>
                            <button type="button" class="btn-off" onClick="articleDel('${boardCode}', ${freeArticle.aid});" class="s-btn-off">게시글삭제</button>
                        </div>
            ...
            <div class="p-lr20">
                ...
                <div style="margin-top: 20px;">
                    ...
                    <input type="hidden" id="boardCode" name="boardCode" value="${boardCode}" />
                     <input type="hidden" name="aid" value="${freeArticle.aid}" />
...
<script>
    function checkComment(){ //동작 기능만 만든 것 -> 실행 아님
       ...
       $.ajax({
           url : "${pageContext.request.contextPath}/freeComment/grp_comment_register",
           type : "POST",
           data : formData,
           success : function(resData){
            alert("댓글이 등록되었습니다.");
	...
	function listComment(){
		...
		$.ajax({
	           url : "${pageContext.request.contextPath}/freeComment/grp_comment_list",
	           type : "POST",
	...
	function deleteComment(cid){
		...
		if(confirm(msg) ){ //confirm은 boolean 타입
			...
			$.ajax({
		           url : "${pageContext.request.contextPath}/freeComment/grp_comment_delete",
		           type : "POST",
		           data : formData,
		           success : function(resData){
		            alert("댓글이 삭제되었습니다.");
	...
	function modifyComment(cid){
		...
		$.ajax({
	           url : "${pageContext.request.contextPath}/freeComment/grp_comment_modify",
	           type : "POST",
	           data : formData,
	           success : function(resData){
	            alert("댓글이 수정되었습니다.");
