...
<!------------------------------------------------ 게시판생성 끝--------------------------------------- -->
							...
							<div class="search-wrap flex flex-justify  m-b5 bg-white">
								...
								<div class="flex flex-justify"  >
									<form method="POST" class="flex flex-justify " action="${pageContext.request.contextPath }/freeboard">
                            ...
                            <table>
                                ...
                                <tr class="center font14 weight700">
                                    ...
                                    <!-- data-uid : 가져오려는 값의 ID -->
                                        <input type="checkbox"  name="chk" class="chk" data-uid="${freeboard.boardCode }"/>
                                    </td>
                                    <td>${freeboard.team_name}</td>
                                    <td>${freeboard.boardCode}</td>
                                    <td>${freeboard.boardColor}</td>
                                    <td>${freeboard.boardTitle }</td>
                                    <td>${freeboard.boardRead }</td>
                                    <td>${freeboard.boardWrite }</td>
                                    <td>${freeboard.boardReply }</td>
                                    <td>${freeboard.boardDown }</td>
                                  	<td>${freeboard.boardReference }</td>
                                    <td>
                                        <button class="s-btn-on" onClick="location.href='${pageContext.request.contextPath }/freeArticle/grp_article_list?boardCode=${freeboard.boardCode }'">보기</button>
                                        <button class="s-btn-off" onClick="boardDel('${freeboard.boardCode}');">삭제</button>
                                    </td>
                                </tr>
                                </c:forEach>
								...
								<c:if test = "${count > 0 }">
									...
									<c:choose>
										<c:when test="${curPage > 1 }">
											<span class="page ">
											<a href="${pageContext.request.contextPath }/freeboard?curPage=1&searchOpt=${searchOpt}&words=${words}">
									...
									<c:choose>
										<c:when test="${curPage > 1 }">
											...
											<a href="${pageContext.request.contextPath }/freeboard?curPage=${curPage-1 }&searchOpt=${searchOpt}&words=${words}">
									...
									<c:forEach begin="${blockBegin }" end="${blockEnd }" var="num"> <!--시작, 끝, 변수명  -->
										...
										<c:if test="${selected != num }">
											<a href="${pageContext.request.contextPath }/freeboard?curPage=${num }&searchOpt=${searchOpt}&words=${words}">
...
<!--------------------------------------한칸 뒤로-------------------------------------------------------------->
									...
									<!--맨 뒤로  -->
										...
										<c:when test="${curPage != totalPage }">
											...
											<a href="${pageContext.request.contextPath }/freeboard?curPage=${totalPage}&searchOpt=${searchOpt}&words=${words}">
...
function loadPost(){
	$.ajax({
		url : "${pageContext.request.contextPath }/freeboard/grp_get_post",
		type : "post",
		data : "",
...
<script>
	$(function(){
		$("#btn").click(function(){
			...
			$.ajax({
				url : "${pageContext.request.contextPath }/freeboard/grp_freeboard_insert",
				type : "post",
				data : $("#frm").serialize(),
				success : function(resData){
...
<script>
	function boardDel(boardCode){
		...
		if(confirm(msg)){  //확인 클릭
			...
			$.ajax({
				url : "${pageContext.request.contextPath }/freeboard/grp_freeboard_delete",
				type : "post",
				data : formData,
				success : function(resData){
					if(resData == "success"){
...
<script>
	function boardModify(boardCode){
		...
		if(confirm(msg)){  //확인 클릭
			...
			$.ajax({
				url : "${pageContext.request.contextPath }/freeboard/grp_freeboard_modify",
				type : "post",
				data : formData,
				success : function(resData){
					if(resData == "success"){
