                        ...
                        <c:if test="${modifyArticle.fileOriName != null}">
	                        <tr>
	                            <td colspan="3" class="td-90 p-lr3 weight700">
									<span class="kr">기존 파일 : <a href="/freeArticle/download?aid=${modifyArticle.aid}&boardCode=${boardCode}">${modifyArticle.fileOriName}</a></span>
	                            </td>
	                        </tr>
                        </c:if>
                    </table>
...
<script>
	function articleModify(boardCode){
		...
		if(confirm(msg)){  //확인 클릭
			...
			$.ajax({
				url : "${pageContext.request.contextPath }/freeArticle/grp_article_modify",
				type : "post",
				data : formData,
